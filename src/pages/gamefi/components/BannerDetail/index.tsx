...
const BannerDetail = (props: objectT) => {
  ...
  // const imgList = datas?.gameInfo?.gamePicture?.split(',') || [errorImg];
  const intl = useIntl();
  ...
  const [mediaList, setMediaList] = useState<objectT[]>([]);
  useEffect(() => {
    const imgs = datas?.gameInfo?.gamePicture?.split(',') || [];
    const videos = datas?.gameInfo?.gameVideo?.split(',') || [];
    const list: SetStateAction<objectT[]> = [];
    videos.length > 0 &&
                ...
                <SwiperSlide key={index}>
                  {item.type == 'img' ? (
                    ...
                    <div className={styles['videos']} key={index}>
                      <video controls muted={index == 0} autoPlay={index == 0}>
                        <source src={item.src} />
                      </video>
                    </div>
                  )}
      ...
      <aside className={styles['intro-box']}>
        ...
        <section className={styles.lines}>
          ...
          <div className={styles.line}>
            ...
            <span className={styles.linetext}>{datas.languageText}</span>
          </div>
          <div className={styles.line}>
            <label className={styles.linelabel}>
              {GAMEFI_DETAIL_BANNER_Platform}
            </label>
            <span className={styles.linetext}>{datas.platformText}</span>
          </div>
        </section>
        {/* <section className={styles.lines} >
          <div className={styles.line}>
